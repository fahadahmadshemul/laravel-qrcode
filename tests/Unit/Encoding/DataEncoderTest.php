...
final class DataEncoderTest extends TestCase
{
    ...
    public function test_hello_world_level_m_reference_codewords(): void
    {
        ...
        $codewords = $this->encoder->encode('HELLO WORLD', 'M', null, EncodingMode::Byte);
    ...
    {
        $this->assertCount(19 + 7, $this->encoder->encode('A', 'L', null, EncodingMode::Byte));
        $this->assertCount(16 + 10, $this->encoder->encode('A', 'M', null, EncodingMode::Byte));
        $this->assertCount(13 + 13, $this->encoder->encode('A', 'Q', null, EncodingMode::Byte));
        $this->assertCount(9 + 17, $this->encoder->encode('A', 'H', null, EncodingMode::Byte));
    }
    ...
    public function test_pad_codewords_alternate_ec_11(): void
    {
        ...
        $codewords = $this->encoder->encode('A', 'H', null, EncodingMode::Byte);
    ...
    public function test_payload_exceeding_capacity_throws(): void
    {
        ...
        // V1-M fits 14 bytes in Byte mode; 15 must overflow V1.
        $this->encoder->encode(str_repeat('a', 15), 'M', VersionTable::get(1), EncodingMode::Byte);
    }

    public function test_capacity_limits_per_level(): void
    {
        // 14 bytes at L/M/Q and 7 at H encode fine in Byte mode.
        $this->assertCount(26, $this->encoder->encode(str_repeat('a', 14), 'L', null, EncodingMode::Byte));
        $this->assertCount(26, $this->encoder->encode(str_repeat('a', 7), 'H', null, EncodingMode::Byte));
    }
